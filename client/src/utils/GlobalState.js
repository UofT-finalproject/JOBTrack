...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    case SAVE_SEARCH:
      return { ...state, searchedJobs: action.searchedJobs, loading: false };

    case CLEAR_SEARCH:
      return { ...state, searchedJobs: [], loading: false };
    ...
    case UPDATE_JOB:
      return {
        ...
        savedJobs: [
          action.updatedJob,
          ...state.savedJobs.filter((job) => job._id !== action.updatedJob._id),
        ],
        sortedFilteredJobs: [
          action.updatedJob,
          ...state.sortedFilteredJobs.filter(
            (job) => job._id !== action.updatedJob._id
          ),
        ],
        loading: false,
      };
