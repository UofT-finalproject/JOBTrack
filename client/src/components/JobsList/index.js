...
function JobsList() {
  ...
  const loadJobs = async () => {
    ...
    API.getSavedJobs(userId).then((res) => {
      dispatch({
        type: UPDATE_JOBS,
        savedJobs: res.data,
      });
  ...
  return (
    <div>
      <Table celled fixed striped selectable>
        <Table.Header>
          <Table.Row>
            <Table.HeaderCell colSpan={2} onClick={() => requestSort("title")}>
              <Icon
                name={"sort" + getClassNamesFor("title")}
            ...
            >
              <Icon
                name={"sort" + getClassNamesFor("company")}
                onClick={() => requestSort("company")}
              />
              Company
            </Table.HeaderCell>
            ...
            >
              <Icon
                name={"sort" + getClassNamesFor("location")}
                onClick={() => requestSort("location")}
              />
              Location
            </Table.HeaderCell>
            ...
            >
              <Icon
                name={"sort" + getClassNamesFor("created_at")}
                onClick={() => requestSort("created_at")}
              />
              Date Posted
            </Table.HeaderCell>
            <Table.HeaderCell colSpan={2} onClick={() => requestSort("status")}>
              <Icon
                name={"sort" + getClassNamesFor("status")}
            ...
            >
              <Icon
                name={"sort" + getClassNamesFor("date_applied")}
                onClick={() => requestSort("date_applied")}
              />
              Date Applied
            </Table.HeaderCell>
        ...
        <Table.Body>
          {items.length > 0 ? (
                ...
                >
                  ...
                  <Table.Cell>
                    <Button
                      ...
                      loading={clickedButtonId === _id && state.loading}
                    >
            ...
            <Table.Row>
              <Table.Cell colSpan={16}>No saved jobs yet...</Table.Cell>
            </Table.Row>
          )}
        </Table.Body>
      </Table>
